...
import cli.arguments as cliarg
import jotjson
import randosettings as rset


class PresetimizerCLIOptionsAG(cliarg.ArgumentGroup):
    _title = 'Presetimizer Options'

    @classmethod
    def arguments(cls) -> Generator[cliarg.Argument, None, None]:
        yield cliarg.Argument(
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(formatter_class=cliarg.SmartFormatter)
    groups: List[Type[cliarg.ArgumentGroup]] = [PresetimizerCLIOptionsAG]
    groups.extend(cliarg.ALL_GENERATION_AG)
    for ag in groups:
...
def build_preset(args: argparse.Namespace) -> Dict[str, rset.JSONType]:
    ...
    if args.input_preset:
        ...
        preset = cliarg.RandomizerCLIOptionsAG.load_preset(args.input_preset)
    ...
    def _to_jot_json(self: rset.Settings) -> Dict[str, Any]:
        ...
        # only include these settings when args related were explicitly passed or from inherited preset
        ...
        if 'tab_settings' in presettings or any(arg.arg in args for arg in cliarg.TabSettingsAG.arguments()):
            jot['tab_settings'] = self.tab_settings
        return jot

    settings = cliarg.args_to_settings(args)
